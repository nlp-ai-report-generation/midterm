...
단순한 "반응 크기(magnitude)"가 아니라, **뇌의 어떤 기능 영역에서** 반응이 강한지를 해석하는 레이어다.

TRIBE v2는 10,242개 정점별 예측을 제공하고, 이 정점들은 Desikan-Killiany atlas 기반으로 해부학적 영역에 매핑되어 있다. 이 영역들을 5개 기능 카테고리로 묶어서 프로필을 구성한다.

| 카테고리 | 뇌 영역 | 의미 | 신경과학 근거 |
|---------|---------|------|-------------|
| **언어 처리** | 측두엽 (상측두이랑, Heschl 이랑, 실비우스열 주변) | 강의 언어를 듣고 의미를 해석 | Wernicke 영역 — 언어 이해의 핵심 (Wernicke, 1874; Geschwind, 1970) |
| **실행 통제** | 전전두엽 (전두극, 전대상회, 보조운동영역) | 정보를 정리하고 개념을 통합 | DLPFC — 작업기억, 인지적 유연성, 메타인지 (Goldman-Rakic, 1995; Miller & Cohen, 2001) |
| **시각 처리** | 후두엽 (설상회, 새발톱고랑, 설회) | 슬라이드·도표·시각 자료 처리 | 일차시각피질 V1 + 복측경로 (Hubel & Wiesel, 1962) |
| **주의 전환** | 두정엽 (중심뒤이랑, 두정간고랑, 모서리위이랑) | 주의 방향 전환, 공간/수리 사고 | Posner 주의 네트워크 — 방향 전환 시 두정엽 활성화 (Posner & Petersen, 1990) |
| **맥락 연결** | DMN (쐐기앞소엽, 후대상회, 각이랑, 측두극) | 맥락 연결, 자기 참조적 사고 | Default Mode Network — Raichle (2001); mind-wandering 시 활성화 (Buckner, 2008) |

#### 프로필 패턴 해석

| 프로필 패턴 | 조건 | 교육적 의미 |
|------------|------|------------|
| **능동적 이해** | 언어 처리 + 실행 통제가 모두 높음 | 설명을 듣고 적극적으로 정리·통합하는 중 — 가장 이상적 |
| **수동적 청취** | 언어 처리만 높고 나머지 낮음 | 듣고는 있으나 깊은 인지적 처리가 약함 — ICAP Passive 수준 |
| **인지적 통합** | 실행 통제가 가장 높음 | 복잡한 개념을 비교·판단·정리하는 중 — 정보 밀도가 높은 구간 |
| **맥락 연결 / 이탈 가능** | DMN이 가장 높음 | 앞뒤 맥락을 연결하는 깊은 사고이거나, mind-wandering 가능 |
| **주의 재정렬** | 주의 전환이 가장 높음 | 새로운 내용에 주의를 돌리는 중 — 전환 구간에서 자주 나타남 |
| **시각 정보 처리** | 시각 처리가 가장 높음 | 화면 자료에 집중하는 구간 |

#### 왜 magnitude만 보면 안 되는가

기존 Attention proxy는 `magnitude * 0.65 + change * 0.35`로 계산한다. 이것은 **전체 cortical response의 크기**를 하나의 숫자로 압축한 것이다.

문제는:
- magnitude가 같더라도, 언어 영역에서 반응한 것과 DMN에서 반응한 것은 교육적 의미가 완전히 다르다
- 언어+실행 통제 동시 활성 = 능동적 이해 vs 언어만 활성 = 수동적 청취 — magnitude는 같을 수 있다
- DMN 활성이 높으면 mind-wandering일 수 있지만, magnitude로는 "반응이 있다"로만 보인다

기능 프로필은 이 문제를 해결하여, **같은 크기의 반응이라도 어떤 영역에서 나왔는지에 따라 다른 해석**을 제공한다.
