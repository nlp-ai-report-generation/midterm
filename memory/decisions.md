...
- 이유: 일부 날짜는 JSON이 중간 강의 내용을 `break`로 삼키거나 마지막 10분만 남기는 식으로 부분 커버 상태가 생겼고, 이 상태로는 강의 흐름 분석과 프론트 탐색이 모두 왜곡된다.
- 결과: 이후 sections JSON 검수는 `첫 start = 원문 첫 줄`, `마지막 end = 원문 마지막 줄`, `break = 실제 20분+ 공백 구간`을 기본 검증 규칙으로 삼는다.

### 첫 주 sections JSON도 전체 타임라인으로 재독해 복원한다

- 결정: `2026-02-02`~`2026-02-06`처럼 초기에 부분만 커버되던 sections JSON은 원문 txt를 처음부터 끝까지 다시 읽어 전체 흐름을 복원한다.
- 이유: 부분 커버 상태를 유지하면 break가 실제 발화를 흡수하고, downstream 분석에서 강의 흐름이 왜곡된다.
- 결과: 첫 주 5개 파일을 다시 확장했고, break/점심/실습 전환이 원문 기준으로 재배치됐다.

### 구간 JSON의 break는 추정이 아니라 실제 공백 시작 시각에 맞춘다

- 결정: `frontend/public/data/lectures/YYYY-MM-DD-sections.json`에서 `break` 구간은 단순히 직전 섹션의 끝이 아니라, 원문 STT의 실제 시간 공백이 시작되는 시각에 맞춰 끊는다.
- 이유: 점심 직전이나 쉬는시간 직전의 짧은 설명이 `break`에 흡수되면 실제 강의 내용이 잘리고, 섹션 라벨과 내용이 어긋난다.
- 결과: `2026-02-09`처럼 점심 직전의 실제 발화가 남아 있는 경우 `break` 시작을 11:58 계열로 옮기고, 공백 앞뒤 내용을 별도 섹션으로 복원했다.
