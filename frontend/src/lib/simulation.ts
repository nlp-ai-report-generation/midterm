...
export function metricTone(value: number, metric?: "attention" | "load" | "novelty") {
  if (metric === "load") {
    ...
    if (value >= 70) return "var(--color-caution)";
    if (value >= 45) return "var(--color-flow)";
...
export function metricLevel(value: number, metric: MetricType): MetricLevel {
  if (metric === "attention") {
    ...
    return { label: "이탈 위험", description: "집중이 풀리고 있을 가능성이 높아요", color: "var(--color-caution)", bgColor: "rgba(245,158,11,0.10)" };
  }

  if (metric === "load") {
    if (value >= 85) return { label: "과부하", description: "정보가 너무 많아 처리 한계에 가까워요", color: "var(--color-error)", bgColor: "rgba(239,68,68,0.10)" };
    if (value >= 70) return { label: "높은 밀도", description: "정보 밀도가 높아 부하가 커지는 중", color: "var(--color-caution)", bgColor: "rgba(245,158,11,0.10)" };
    if (value >= 45) return { label: "최적 도전", description: "적절한 어려움 — 학습에 가장 좋은 구간", color: "var(--color-flow)", bgColor: "rgba(34,197,94,0.10)" };
    if (value >= 25) return { label: "여유", description: "쉬운 내용 — 부하가 낮은 구간", color: "var(--grey-600)", bgColor: "rgba(71,85,105,0.08)" };
    return { label: "너무 쉬움", description: "자극이 부족해 깊은 학습이 어려워요", color: "var(--color-caution)", bgColor: "rgba(245,158,11,0.10)" };
  }

  // novelty
  if (value >= 85) return { label: "맥락 끊김", description: "변화가 너무 커서 혼란이 올 수 있어요", color: "var(--color-error)", bgColor: "rgba(239,68,68,0.10)" };
  if (value >= 70) return { label: "급변", description: "큰 변화 — 잘 연결해주면 학습에 도움돼요", color: "var(--color-caution)", bgColor: "rgba(245,158,11,0.10)" };
