...
멋쟁이사자처럼 AI 스쿨 KDT 백엔드 자바 21기 15개 강의를 대상으로 3개 AI 모델(GPT-4o mini, Claude Opus, Claude Sonnet)이 교차 평가한 결과를 대시보드에서 바로 확인할 수 있어요.
...
> 강의 녹취록이 들어오면 → 60분 윈도우/30분 hop으로 쪼개고 → 5개 카테고리 평가자가 동시에 읽고 점수를 매기고 → 가중 평균을 내고 → 리포트를 만들어요.
...
```
원본 트랜스크립트 (3시간, 2000줄)
    ↓ chunk_by_time_window(window=60분, hop=30분)
    ↓
[청크 0] 00:00 ~ 01:00  (도입부)
[청크 1] 00:30 ~ 01:30  (← 30분 겹침)
[청크 2] 01:00 ~ 02:00
    ...
[청크 N] 마지막 구간
```
...
50% hop(30분 겹침)을 두는 이유도 있어요. 실험 결과 hop 50%가 맥락 연속성과 안정성 모두에서 가장 좋았어요(안정 항목 비율 77.8%).
...
### 실험 3. 윈도우 길이에 따라 점수가 달라질까? (30분 vs 60분 vs 120분)

| 카테고리 | 최대 편차 |
|----------|:---------:|
| 예시·실습 연계 | **0.667** (가장 민감) |
| 언어 표현 품질 | **0.238** (가장 안정) |

> **60분 윈도우**가 세밀도와 맥락의 최적 균형이에요.

### 실험 4. Hop 크기에 따라 안정성이 달라질까? (50% vs 75% vs 90%)
...
> **Hop 50%**가 맥락 연속성을 유지하면서 가장 안정적이에요.
...
| 소개 | /presentation | 프로젝트 소개 페이지 (문제 정의 → 파이프라인 → 신뢰도 → TRIBE v2 → 결과) |
...
[TRIBE v2](https://github.com/facebookresearch/tribev2)는 Meta AI/FAIR이 개발한 뇌 인코딩 모델이에요. **25명 피험자의 451.6시간 실제 fMRI 데이터**로 훈련되었고, **Brain-Score 벤치마크 종합 1위**를 달성했어요.

#### TRIBE v2 성능

| 항목 | 내용 |
|------|------|
| **벤치마크** | Brain-Score (brain-score.org) — 시각/청각/언어 신경 반응 예측 종합 평가 |
| **순위** | 종합 1위 (발표 시점 기준) |
| **훈련 데이터** | 25명 피험자, 451.6시간 fMRI |
| **뇌 해상도** | 10,242개 cortical surface vertex (fsaverage5) |
| **인코더** | LLaMA 3.2 (텍스트) + Wav2Vec-BERT (오디오) + DINOv2 (비전) |
| **차별점** | 최초의 3모달(시각+청각+언어) 뇌 인코딩 모델. 멀티모달 학습이 단일 모달 예측도 향상시킴 |
| **출처** | d'Ascoli et al. (2024). *A foundation model of vision, audition, and language for in-silico neuroscience.* Meta AI Research |
...
| Phase | 내용 | 상태 |
|-------|------|:----:|
| **1. 신뢰성 검증** | 4개 실험 완료 (ICC 0.877, 청크 p<0.001, 윈도우 60분, Hop 50%) | ✅ |
| **2. TRIBE v2 시뮬레이션** | 뇌 반응 시뮬레이션 3/15 강의 완료, 3D 히트맵 + ROI 처방 | 🔄 |
| **3. 프로젝트 소개 페이지** | Apple 디자인 기반 애니메이션 프레젠테이션 페이지 | ✅ |
| **4. 외부 벤치마크** | 유튜브 강의 자막 수집 → 동일 파이프라인 평가 → 비교 리포트 | ⬜ |
| **5. 파이프라인 고도화** | 프롬프트 버저닝, 비용 최적화, 실시간 평가 | ⬜ |
